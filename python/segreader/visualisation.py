"""Convert pipeline intermediates into displayable BGR images."""
...
def boxes(bin_img: np.ndarray, digit_boxes: list, digits: list) -> np.ndarray:
    out = binary(bin_img)
...
def localize(debug_img: np.ndarray, method_used: str, found: bool) -> np.ndarray:
    ...
    Adds a top stripe that documents which of the three auto strategies
    succeeded (color → brightness → contour) and whether a display region
    was actually found.
    """
    out = debug_img.copy()
    h, w = out.shape[:2]
